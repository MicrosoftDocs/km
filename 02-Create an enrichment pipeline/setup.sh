...
az cognitiveservices account create --kind CognitiveServices --location $location --name cog${clean_uuid:0:18} --sku S0 --resource-group rg${clean_uuid:0:18} --output none
cogkeys=$(az cognitiveservices account keys list --name cog${clean_uuid:0:18} --resource-group rg${clean_uuid:0:18})
...
echo "Cognitive Services account: cog${clean_uuid:0:18} {"
set -- $cogkeys
echo " \"Key\": $3"
echo "}"
